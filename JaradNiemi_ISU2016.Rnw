...
\item Simulation model
  ...
  \item edgeR: negative binomial model with data-based gene-specific parameters
  \end{itemize}
\item Inference
  \begin{itemize}
  \item edgeR: non-hierarchical except for overdispersion, negative binomial model
  \item fully Bayes: Bayesian analysis with hierarchical count regression model 
  ...
  \item eBayes (Oracle): empirical Bayesian analysis with hierarchical count regression model with true values for the hyperparameters
  \end{itemize}
\item Data size
  \begin{itemize}
  \item $G=30\,000$
