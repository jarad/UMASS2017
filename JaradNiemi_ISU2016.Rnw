...
The computation for this hierarchical overdispersed count regression model is provided in the following three R packages:
...
\item $\xi_{g\ell} \ind Exp(k_\ell) \implies \beta_{g\ell} \ind Laplace(\theta_\ell, \sigma^2_\ell/2k_\ell)$
\item $\xi_{g\ell} \ind Ca^+(0,1) \implies \beta_{g\ell} \ind Horseshoe(\theta_\ell,\sigma_\ell)$ (Carvalho, Polson, and Scott 2010)
\end{itemize}
\pause
...
\]
i.e. a Dirichlet process prior with concentration parameter $a>0$ and base measure $F_0$ on $\mathbb{R}^{L+1}$. 
Related to work by Liu, Wang, and Liu (2015). 
\end{frame}
...
\[ 
f(\theta) = \sum_{k=1}^\infty \pi_k \delta_{\tilde\theta_k}(\theta)
\]
where $\tilde\theta_k \sim F_0$, $\pi_k= \pi'_k \prod_{i=1}^{k-1} (1-\pi'_i)$, and $\pi'_k \ind Be(1,\alpha)$.

\vspace{0.1in} \pause

We can approximate the infinite sum with a finite sum of $K$ components with $\pi_K = 1-\sum_{k=1}^{K-1} \pi_k$ and introduce labels $z_g\in \{1,\ldots,K\}$ that indicating component membership for gene $g$. 
...
\item $p(\tilde\theta|\ldots)$ which is 
  ...
  \item requires sufficient ``statistics'' of the genes in each component.
  \end{itemize}
\end{itemize}
\end{frame}
...
\begin{itemize}
\item how do we construct a model matrix for arbitrary scenarios (Lithio and Nettleton 2015) and
